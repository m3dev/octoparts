...
class InstrumentedHttpClientHttpClientSpec extends FunSpec with Matchers {
  it("should not fail (only warn) if 2 clients share the same name") {
    val clients = for (i <- 1 to 2) yield new InstrumentedHttpClient("A", 1, 5.seconds, 5.seconds)
    try {
      clients.foreach {
  ...
  it("should remove gauges on close") {
    val client = new InstrumentedHttpClient("A", 1, 5.seconds, 5.seconds)
  ...
  it("should be able to close and recreate a client with the same name") {
    new InstrumentedHttpClient("A", 1, 5.seconds, 5.seconds).close()
    val client = new InstrumentedHttpClient("A", 1, 5.seconds, 5.seconds)
    try {
  ...
  it("should set the connection pool size") {
    val client = new InstrumentedHttpClient("A", 42, 5.seconds, 5.seconds)
    try {
