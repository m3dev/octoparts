...
trait HandlerMocks {
  ...
  val mockVoidHandler = new Handler {
    ...
    def process(args: HandlerArguments) = Future.successful(PartResponse(partId, partId, contents = Some("it worked")))
  }
  val mockErrorHandler = new Handler {
    val partId = "errorHandler"
    def process(args: HandlerArguments) = Future.failed(new RuntimeException)
  }
  val mockPartResponseWithErrorHandler = new Handler {
    val partId = "errorHandler"
    def process(args: HandlerArguments) = Future.successful(PartResponse(partId, partId, errors = Seq("SomeException")))
  }
