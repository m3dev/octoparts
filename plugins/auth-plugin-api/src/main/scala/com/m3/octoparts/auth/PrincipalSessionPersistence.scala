  ...
  def extractPrincipalFromPlaySession(session: Session): Option[Principal] = {
    session.get(SessionKey).flatMap { sessionValue =>
      Try(Json.parse(sessionValue)) match {
        case Failure(e) =>
          LTSVLogWriter.warn("Rejecting invalid principal found in session (not valid json)" -> sessionValue)
          None
        case Success(json) =>
          json.validate[Principal] match {
            case JsError(_) =>
              LTSVLogWriter.warn("Rejecting invalid principal found in session (unexpected json)" -> sessionValue)
              None
